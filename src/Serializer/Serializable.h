...
public:
    Serializable() {
        static_assert(HasStaticName<Derived>, "Derived must provide: static const std::string& static_name()"); // check concept for static_name

        static bool factoryRegistered = false;
        if (!factoryRegistered) { // used to create the child classes during deserialization
    ...
    void registerField(const std::string &field_name, T Derived::*member_ptr, F converter) {
        ...
        Loader fieldLoader = [conv_ptr, member_ptr](std::shared_ptr<ISerializable> obj, const std::string &s) -> bool {
