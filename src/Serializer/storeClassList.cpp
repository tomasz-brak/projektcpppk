  ...
  if(std::ofstream outFile(filePath); outFile.is_open())
    {
      outFile << str;
      outFile.close();
      std::cout << "Dane dla " << filename << " zostaly zapisane pomyslnie !!!"
                << std::endl;
      return true;
    }
...
bool isEscaped(const std::string::const_iterator &begin,
...
void format(const std::string &filename)
{
  ...
  auto appendIndent = [&](int levels) {
    if(levels > 0)
      out.append(static_cast<size_t>(levels * 4), ' ');
  };
  ...
  if(indent != 0)
    {
      std::cerr << "Format Error! mismatched braces/brackets (indent="
                << indent << "). Not saving!\n";
      return;
    }
...
ParseToken(const std::string::const_iterator ch,
           ...
           std::stack<std::size_t> &tokenStack)
{
  ...
  switch(*ch)
    {
    case '{': tokenStack.emplace(std::distance(stringBegin, ch)); break;
    case '[': tokenStack.emplace(std::distance(stringBegin, ch)); break;
    case '}':
      if(!tokenStack.empty())
        {
  ...
  for(auto it = input.begin(); it != input.end(); ++it)
    {
      ...
      if(auto closes = ParseToken(it, parserState, input.begin(), tokenStack);
         closes.has_value())
        {
          const auto opened_idx = closes.value();
          if(opened_idx < parsedChars.size())
            {
              parsedChars[opened_idx]->status->closed_by
                = std::distance(input.begin(), it) - opened_idx;
            }
        }
...
assembleTokenTree(std::span<std::shared_ptr<charData>> chars,
      ...
      if(chars[i]->status->closed_by.has_value())
        {
          ...
          auto inner_span
            = chars.subspan(i + 1, chars[i]->status->closed_by.value() - 1);
...
assembleTokenTree(std::vector<std::shared_ptr<charData>> &vec,
...
void loadClass(const std::shared_ptr<Token> &tokenTree,
               std::vector<std::shared_ptr<ISerializable>> &data)
{
  assert(!tokenTree->children.empty()); // Root token is not empty
  assert(!tokenTree->FileName.empty()); // Root token has a filename
  assert(tokenTree->children[0]->type
         == ARRAY); // Root contains an array of classes

  const std::string &className = tokenTree->FileName;
  if(!loaderRegistry().contains(className))
    {
      std::cerr << "No loaders registered for class: " << className
                << std::endl;
      return;
    }
  ...
  if(!factoryRegistry().contains(className))
    {
      std::cerr << "No factory registered for class: " << className
                << std::endl;
      return;
    }
  ...
  for(const auto &datapoint : tokenTree->children[0]->children)
    {
      ...
      assert(datapoint->type
             == DICT); // Class is represented by key-value pairs
      ...
      // Iterate over key-value pairs in the JSON object to populate the
      // instance.
          ...
          if(loaders.contains(key))
            {
              ...
              if(!fieldLoader(instance, dict_child->value->contentAsString()))
                {
                  std::cerr << "Loader failed for field: " << key
                            << " in class " << className << std::endl;
                }
            }
          else
            {
              std::cerr << "No loader for field: " << key << " in class "
                        << className << std::endl;
            }
        }
               ...
               const std::string &dataDir)
{
  ...
  if(std::ifstream file(filename); !file.is_open())
    {
      std::cout << "Nie udalo sie otworzyc pliku do odczytu: " << filename
                << std::endl;
      return false;
    }
  else
