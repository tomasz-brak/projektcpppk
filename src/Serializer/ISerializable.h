...
using Loader
  = std::function<bool(std::shared_ptr<ISerializable>, const std::string &)>;
using Factory = std::function<std::shared_ptr<ISerializable>()>;
using LoaderRegistry
  = std::unordered_map<std::string, std::unordered_map<FieldName, Loader>>;
...
inline void registerLoader(const std::string &name, Loader loader,
